  ...
  <body>
    ...
    <div class="divcarttable">
        <table class="divcarttable2">
            <tr class="divcartrow">
                <td class="seniortabletd">Pavadinimas</td>
                <td class="seniortabletd">Kaina 1vnt.</td>
                <td class="seniortabletd">Kiekis</td>
                <td class="seniortabletd">Pašalinti</td>
            </tr>
            @foreach ($result as $item)
            <tr>
            
                <td class="seniortabletdinfo">{{ $item['product']->title }}</td>
                <td class="seniortabletdinfo">{{ $item['product']->price }}</td>
                
                <td class="seniortabletdinfo">{{ $item['count'] }}</td>
               
                <td class="seniortabletdinfo">
                  <a href="{{url('deleteorderproduct',$item['orderproduct_id'])}}"  class=" btn-danger">Pašalinti </a>  
                </td>
            </tr>
            @endforeach
        </table>
        <form method="post" action="{{url('confirmorder',$order_id)}}">
          @csrf
          <button style="font-size:30px;" type="submit" class="btn btn-outline-success btn-lg" value="Pateikti užsakymą">Patvirtinti užsakymą</button>
        </form>
